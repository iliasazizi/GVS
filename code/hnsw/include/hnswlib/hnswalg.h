...
////RNG PRUNING ANGLE
        ...
        tableint mutuallyConnectNewElement(const void *data_point, tableint cur_c,
                                       std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst> &top_candidates,
        int level, bool isUpdate, float rng = 0) {
            size_t Mcurmax = level ? maxM_ : maxM0_;

            if(rng<1)
            getNeighborsByHeuristic2(top_candidates, M_);
            else if(rng<10)
                getNeighborsByRNGALPHA(top_candidates, M_, rng);
            else
                getNeighborsByANGLE(top_candidates, M_, rng);
            ...
            // copy the selectednn into curr nout
            {
                ...
                if (!is_cur_c_present) {
                    if (sz_link_list_other < Mcurmax) {
                        ...
                        if(rng<1)
                            ...
                            getNeighborsByRNGALPHA(candidates, Mcurmax, rng);
                        else
                            getNeighborsByANGLE(candidates, Mcurmax, rng);
...
//        static const unsigned char REUSE_MARK = 0x10;
        ...
        void addPoint(const void *data_point, labeltype label, float rng = 0) {
            addPoint(data_point, label,-1, rng);
...
//                        continue;
        ...
        tableint addPoint(const void *data_point, labeltype label, int level, float rng) {
            ...
            if ((signed)currObj != -1) {
                ...
                for (int level = std::min(curlevel, maxlevelcopy); level >= 0; level--) {
                    ...
                    currObj = mutuallyConnectNewElement(data_point, cur_c, top_candidates, level, false,rng);
...
//                        __COUNT_NUM_DISTCAL
        ...
        unsigned char *sax_cache;
        unsigned char *max_sax_cardinalities;
        size_t sax_alphabet_cardinality;
        float mindist_sqrt;

        float getSaxDist(float * qpaa, sax_type * sax){
            return minidist_paa_to_isax_raw_SIMD(qpaa, sax, max_sax_cardinalities,
                                                 sax_bit_cardinality,
                                                 sax_alphabet_cardinality,
                                                 paa_segments, MINVAL, MAXVAL,
                                                  mindist_sqrt);

        }
        sax_type * getSax(const unsigned int i){
            return sax_cache  + paa_segments * i;
        }


        std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst>
        searchBaseLayerSTSax(tableint ep_id, const void *data_point, float * qpaa,size_t ef, querying_stats & stats) const {
            VisitedList *vl = visited_list_pool_->getFreeVisitedList();
            vl_type *visited_array = vl->mass;
            vl_type visited_array_tag = vl->curV;
            ...
            dist_t lowerBound;
                ...
                dist_t dist = fstdistfunc_(data_point, getDataByInternalId(ep_id), dist_func_param_);stats.distance_computations_bsl++;
                lowerBound = dist;
                top_candidates.emplace(dist, ep_id);
                candidate_set.emplace(-dist, ep_id);
                ...
                for (size_t j = 1; j <= size; j++) {
                    ...
                    if (!(visited_array[candidate_id] == visited_array_tag)) {
                        ...
                        visited_array[candidate_id] = visited_array_tag;
                         ...
                         distsax = minidist_paa_to_isax_raw_SIMD(qpaa, sax_cache + paa_segments * candidate_id, max_sax_cardinalities,
                                                                      sax_bit_cardinality,
                                                                      sax_alphabet_cardinality,
                                                                      paa_segments, MINVAL, MAXVAL,
                                                                      mindist_sqrt) ;stats.saxdist_computations_bsl++;
#ifdef USE_SSE
                        _mm_prefetch( sax_cache + paa_segments * candidate_set.top().second ,_MM_HINT_T0);
#endif

                        if(distsax<lowerBound || top_candidates.size() < ef){
                            char *currObj1 = (getDataByInternalId(candidate_id));
                            dist_t dist = fstdistfunc_(data_point, currObj1, dist_func_param_);
                            stats.distance_computations_bsl++;
                            if (top_candidates.size() < ef || lowerBound > dist) {
        ...
        float * searchGraphSax(const void *query_data, float * qpaa, size_t k, querying_stats & stats, int simslvl) const {
            auto f = std::chrono::high_resolution_clock::now();
                ...
                for (int level = maxlevel_; level > 0; level--) {
                    ...
                    while (changed) {
                        ...
                        for (int i = 0; i < size; i++) {
                            ...
                            float saxdist = minidist_paa_to_isax_raw_SIMD(qpaa, sax_cache + paa_segments * cand,
                                                                          max_sax_cardinalities,
                                                                          sax_bit_cardinality,
                                                                          sax_alphabet_cardinality,
                                                                          paa_segments, MINVAL, MAXVAL,
                                                                          mindist_sqrt);
                            stats.saxdist_computations_hsl++;
                            if (saxdist < curdist) {
                                stats.distance_computations_hrl++;
                                dist_t d = fstdistfunc_(query_data, getDataByInternalId(cand), dist_func_param_);
                                if (d < curdist) {
            ...
            top_candidates=searchBaseLayerSTSax(
                    currObj, query_data, qpaa,std::max(ef_, k), stats);
