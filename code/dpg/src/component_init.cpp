...
#pragma omp parallel for
        ...
        for (unsigned i = 0; i < index->getBaseLen(); i++) {
            ...
            for (unsigned j = 0; j < range; j++) {
                ...
                float dist = index->getDist()->l2opt(index->getBaseData() + i * index->getBaseDim(),
...
#pragma omp parallel for
        ...
        for (unsigned i = 0; i < index->getBaseLen(); i ++) {
            ...
            for (unsigned j = 0; j < index->getBaseLen(); j ++) {
                ...
                float dist = index->getDist()->l2opt(index->getBaseData() + index->getBaseDim() * i,
        ...
        if (node->Lchild != nullptr && node->Rchild != nullptr) {
            ...
            for (; start < end; start++) {
                ...
                for (size_t i = leaf->StartIdx; i < leaf->EndIdx; i++) {
                    ...
                    float dist = index->getDist()->l2opt(index->getBaseData() + tmpfea * index->getBaseDim(),
...
#pragma omp for schedule(dynamic, 128)
    ...
    void ComponentInitNSW::SearchAtLayer(Index::HnswNode *qnode, Index::HnswNode *enterpoint, int level,
        ...
        float d = index->getDist()->l2opt(index->getBaseData() + qnode->GetId() * index->getBaseDim(),
        ...
        while (!candidates.empty()) {
            ...
            for (const auto &neighbor : neighbors) {
                ...
                if (visited_list->NotVisited(id)) {
                    ...
                    d = index->getDist()->l2opt(index->getBaseData() + qnode->GetId() * index->getBaseDim(),
...
#pragma omp for schedule(dynamic, 128)
    ...
    void ComponentInitHNSW::Link(Index::HnswNode *source, Index::HnswNode *target, int level) {
        ...
        for (const auto &neighbor : neighbors) {
            float tmp = index->getDist()->l2opt(index->getBaseData() + source->GetId() * index->getBaseDim(),
                                                  index->getBaseData() + neighbor->GetId() * index->getBaseDim(),
                                                  index->getBaseDim());
...
#pragma omp task
    ...
    const float ComponentInitANNG::MedianSumm(const std::vector<unsigned>& SS1, const std::vector<unsigned>& SS2, const unsigned& v) const
    {
        ...
        if(!SS1.empty())
        {
            ...
            c_max_distance = index->getDist()->l2opt(index->getBaseData() + index->getBaseDim() * (*it_obj),
            ...
            {
                c_current_distance = index->getDist()->l2opt(index->getBaseData() + index->getBaseDim() * (*it_obj),
                                                               index->getBaseData() + index->getBaseDim() * v,
                                                               index->getBaseDim());
        ...
        if(!SS2.empty())
        {
            ...
            c_min_distance = index->getDist()->l2opt(index->getBaseData() + index->getBaseDim() * (*it_obj),
                                                       index->getBaseData() + index->getBaseDim() * v,
                                                       index->getBaseDim());
            ++it_obj;
                                        ...
                                        const std::vector<unsigned>::const_iterator it_end)
    {
        ...
        while(it_obj != it_end)
        {
            current_distance += index->getDist()->l2opt(index->getBaseData() + index->getBaseDim() * (*it_obj),
    ...
    void ComponentInitANNG::RedistributeAmongLeafNodes(const Index::VPNodePtr& parent_node, const unsigned& new_value)
    {
        ...
        std::sort(S.begin(), S.end(), [m_main_val, tmp_index](unsigned val1, unsigned val2) {
            float dist1 = tmp_index->getDist()->l2opt(tmp_index->getBaseData() + tmp_index->getBaseDim() * m_main_val,
                                                        tmp_index->getBaseData() + tmp_index->getBaseDim() * val1,
                                                        tmp_index->getBaseDim());
            float dist2 = tmp_index->getDist()->l2opt(tmp_index->getBaseData() + tmp_index->getBaseDim() * m_main_val,
    ...
    Index::VPNodePtr ComponentInitANNG::MakeVPTree(const std::vector<unsigned>& objects, const Index::VPNodePtr& parent){
        ...
        {
            float dist = index->getDist()->l2opt(index->getBaseData() + index->getBaseDim() * new_node->get_value(),
        ...
        if(!root->get_branches_count() && !root->get_leaf_node()){
            ...
            while (!c_current_node->get_leaf_node() && c_current_node.get()) {
                ...
                for (size_t c_pos = 0; c_pos < c_current_node->m_mu_list.size(); ++c_pos) {
                    ...
                    float dist = index->getDist()->l2opt(index->getBaseData() + index->getBaseDim() * new_value,
            ...
            for (int j = 0; j < (int) TptreeLeafNodes[i].size(); j++) {
                ...
                for (int x = start_index; x < end_index; x++) {
                    for (int y = x + 1; y <= end_index; y++) {
                        ...
                        float dist = index->getDist()->l2opt(index->getBaseData() + index->getBaseDim() * p1,
        ...
        for (int tid = 0; tid < args._T; tid++) {
            ...
            for (int i = istart; i < iend; i++) {
                ...
                for (int k = 0; k < args._DK; k++) {
                    float dist = index->getDist()->l2opt(index->getBaseData() + index->getBaseDim() * indices[i],
...
//            std::cout << args.clusterIdx[i] << std::endl;
        ...
        for (int k = 0; k < args._DK; k++) {
            ...
            float dist = index->getDist()->l2opt(index->getBaseData() + index->getBaseDim() * args.clusterIdx[k],
        ...
        for (int k = 0; k < args._DK; k++) {
            ...
            diff += (index->getDist()->l2opt(args.centers + k * args._D, TCenter, args._D));
        }
...
#pragma omp parallel for
        ...
        std::cerr <<" Min Degree : " << min << ",Max degree : "<<max <<std::endl;
...
//        }
    ...
    ComponentInitHCNNG::create_exact_mst(int *idx_points, int left, int right, int max_mst_degree) {
        ...
        for (int i = 0; i < N; i++) {
            for (int j = 0; j < N; j++)
                if (i != j) {
                    float dist = index->getDist()->l2opt(
    ...
    ComponentInitHCNNG::create_clusters(int *idx_points, int left, int right,
        ...
        if (num_points < minsize_cl) {
            ...
            for (int i = 0; i < num_points; i++) {
                dx[i] = std::make_pair(
                        index->getDist()->l2opt(index->getBaseData() + index->getBaseDim() * idx_points[x],
                                                  index->getBaseData() +
                                                  index->getBaseDim() * idx_points[left + i],
                                                  index->getBaseDim()), idx_points[left + i]);
                dy[i] = std::make_pair(
                        index->getDist()->l2opt(index->getBaseData() + index->getBaseDim() * idx_points[y],
                                                  index->getBaseData() +
                                                  index->getBaseDim() * idx_points[left + i],
                                                  index->getBaseDim()), idx_points[left + i]);
            }
