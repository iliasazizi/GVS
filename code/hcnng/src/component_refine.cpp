...
#pragma omp parallel for
    for(unsigned i=0; i<c.size(); i++){
        ...
        float dist = index->getDist()->l2opt(index->getBaseData() + c[i] * index->getBaseDim(), index->getBaseData() + j * index->getBaseDim(), index->getBaseDim());
    ...
    bool ComponentRefineKDRG::GreedyReachabilityChecking(const unsigned x, const unsigned y, std::vector<std::vector<Index::SimpleNeighbor>>& cut_graph_, double& dist) {
        double xy = index->getDist()->l2opt(index->getBaseData() + index->getBaseDim() * x,
        ...
        for(const Index::SimpleNeighbor nn : cut_graph_[y]) {
            double d = index->getDist()->l2opt(index->getBaseData() + index->getBaseDim() * x,
