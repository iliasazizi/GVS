...
namespace weavess {
        ...
        while (k < (int) L) {
            ...
            if (pool[k].flag) {
                ...
                for (unsigned m = 0; m < index->getLoadGraph()[n].size(); ++m) {
                    ...
                    float dist = index->getDist()->l2opt(index->getQueryData() + index->getQueryDim() * query,
    ...
    void ComponentSearchRouteNSW::SearchAtLayer(unsigned qnode, Index::HnswNode *enterpoint, int level,
        ...
        float d = index->getDist()->l2opt(index->getQueryData() + qnode * index->getQueryDim(),
        ...
        while (!candidates.empty()) {
            ...
            for (const auto &neighbor : neighbors) {
                ...
                if (visited_list->NotVisited(id)) {
                    ...
                    d = index->getDist()->l2opt(index->getQueryData() + qnode * index->getQueryDim(),
    ...
    void ComponentSearchRouteHNSW::RouteInner(unsigned int query, std::vector<Index::Neighbor> &pool,
        ...
        float d = index->getDist()->l2opt(index->getQueryData() + query * index->getQueryDim(),
            ...
            while (changed) {
                ...
                for (auto iter = neighbors.begin(); iter != neighbors.end(); ++iter) {
                    if(visited[(*iter)->GetId()] != visited_mark) {
                        ...
                        d = index->getDist()->l2opt(index->getQueryData() + query * index->getQueryDim(),
    ...
    //                 float d = index->getDist()->l2opt(index->getQueryData() + index->getQueryDim() * query,
    ...
    void ComponentSearchRouteIEH::RouteInner(unsigned int query, std::vector<Index::Neighbor> &pool,
        ...
        for (size_t j = 0; j < pool.size(); j++) {
            ...
            Index::Candidate2<float> c(neighbor,
                                       index->getDist()->l2opt(&index->test[query][0], &index->train[neighbor][0], index->test[query].size()));
            index->addDistCount();
            cands.insert(c);
        ...
        while (niter++ < iterlimit) {
            ...
            for (size_t j = 0; j < ids.size(); j++) {
                Index::Candidate2<float> c(ids[j], index->getDist()->l2opt(&index->test[query][0], &index->train[ids[j]][0],
    ...
    void ComponentSearchRouteBacktrack::RouteInner(unsigned int query, std::vector<Index::Neighbor> &pool,
        ...
        mp[enter] = 0;
        float dist = index->getDist()->l2opt(index->getBaseData() + index->getBaseDim() * start,
        ...
        while (k < (int)L) {
            ...
            if (pool[k].flag) {
                ...
                for (unsigned m = 0; m < MaxM; ++m) {
                    ...
                    flags[id] = 1;
                    /***
                     * CHANGE WITH OPT DISTANCE CALCULATION
                     * */

                 //   float dist = index->getDist()->l2opt(index->getQueryData() + query * index->getQueryDim(),
                   //                                        index->getBaseData() + id * index->getBaseDim(),
                     //                                      (unsigned)index->getBaseDim());

                    float dist = compare(index->getQueryData() + query * index->getQueryDim(),
                                                           index->getBaseData() + id * index->getBaseDim(),
                                                           (unsigned)index->getBaseDim());
...
//        std::cout << std::endl;
    ...
    void ComponentSearchRouteSPTAG_KDT::KDTSearch(unsigned int query, int node, Index::Heap &m_NGQueue,
        ...
        if (node < 0)
        {
            ...
            m_NGQueue.insert(Index::HeapCell(tmp, index->getDist()->l2opt(index->getQueryData() + index->getQueryDim() * query,
    ...
    void ComponentSearchRouteSPTAG_KDT::RouteInner(unsigned int query, std::vector<Index::Neighbor> &pool,
        ...
        while (!m_NGQueue.empty()) {
            ...
            for (unsigned i = 0; i < node.size(); i++) {
                ...
                float distance2leaf = index->getDist()->l2opt(index->getQueryData() + index->getQueryDim() * query,
    ...
    void ComponentSearchRouteNGT::RouteInner(unsigned int query, std::vector<Index::Neighbor> &pool,
        ...
        while (!unchecked.empty()){
            ...
            for (unsigned neighborptr = 0; neighborptr < neighbors.size(); ++neighborptr){
                ...
                float distance = index->getDist()->l2opt(index->getQueryData() + index->getQueryDim() * query,
