...
//
        unsigned K = final_index_->getParam().get<unsigned>("K_search");
//
...
//weavess::TYPE::ROUTER_GREEDY, weavess::TYPE::L_SEARCH_ASSIGN
...
//            std::cout << "__ROUTER : GUIDED__" << std::endl;
                ...
                for (unsigned i = 0; i < final_index_->getQueryLen(); i++) {
                    ...
                    for(size_t j=0; j < K; j++){
                        printf(" K N°%lu  => Distance : %f | Node ID : %u | Time  : %f | TOTAL DC : %i | Total hops : %i  \n",
                               j+1,std::sqrt(pool[j].distance),pool[j].id,time,0,0);
                        time = 0;
                    }
    ...
    */
    ...
    IndexBuilder *IndexBuilder::save_graph(TYPE type, const char *graph_file) {
    ...
    IndexBuilder *IndexBuilder::load_graph(TYPE type, char *graph_file) {
        ...
        /*if (type == INDEX_NSG || type == INDEX_VAMANA) {
            ...
            */
            if (type == INDEX_HCNNG) {
            size_t K;
                std::cerr <<" Start Index Loading "<< graph_file <<" ...";
            //read file head
            in.read((char*)&(final_index_->nTrees),sizeof(unsigned));
            in.read((char*)&(final_index_->mLevel), sizeof(unsigned));
            in.read((char*)&(K),sizeof(unsigned));
