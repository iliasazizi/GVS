...
*/
          ...
          int numberOfDistanceCalculation = 0;
            ...
            while (!p_space.m_NGQueue.empty()) 
            {
                ...
                if (notDeleted(m_deletedID, gnode.node))
                {
                    ...
                    {
                        p_query.SortResult();
                        numberOfDistanceCalculation = p_space.m_iNumberOfCheckedLeaves;
                        auto t2 = std::chrono::high_resolution_clock::now();
                        float time = (float)(std::chrono::duration_cast<std::chrono::microseconds>(t2 - t1).count() / 1000000.0);
                        if(m_mode){
                            for( int i = 0; i < p_query.m_resultNum; i++){
                                printf("K N°%d  => Distance : %f | Node ID : %d | Time : %f | DC : %d | NH : %d \n", i, sqrt( p_query.GetResult(i)->Dist), p_query.GetResult(i)->VID, time, numberOfDistanceCalculation, numberOfHopes);     
                            }
                        }
                        return;
                    }
            ...
            // printf("the number of distance calculations %d", numberOfDistanceCalculation);
        }
        ...
        {
            printf("Inside RefineSearchIndex \n");
            auto workSpace = m_workSpaceFactory->GetWorkSpace();
            if (!workSpace) {
                workSpace.reset(new COMMON::WorkSpace());
                workSpace->Initialize(max(m_iMaxCheck, m_pGraph.m_iMaxCheckForRefineGraph), m_iHashTableExp);
