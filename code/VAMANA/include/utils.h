...
namespace diskann {
  ...
  inline void load_aligned_bin_impl(std::basic_istream<char>& reader,
    ...
    template<typename T>
    void load_data(const std::string &path, T*& data, const size_t npts,const int dim,size_t& rounded_dim ){// load data with sift10K pattern
        std::ifstream in(path.c_str(), std::ios::binary);
        if(!in.is_open()){std::cout<<"open file error, cannot be opened!"<<std::endl;exit(-1);}
        diskann::cout << "Reading bin file " << path << " ..." << std::flush;
