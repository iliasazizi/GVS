...
//  std::string result_output_prefix(argv[ctr++]);
  Lvec.push_back(std::atoi(argv[ctr++]));
  _u64        num_threads = 1;// std::atoi(argv[ctr]);
...
/*
  ...
  /*
  ///TEST
  std::ifstream reader(query_bin, std::ios::binary | std::ios::ate);
  reader.seekg(0);
  int npts=100,dim=256;
  std::cout << query_bin << std::endl;
  reader.read((char*) &npts, sizeof(int));
  reader.read((char*) &dim, sizeof(int));
  std::cout << npts << " "<< dim <<std::endl;
//  diskann::alloc_aligned(((void**) &query), npts*dim*sizeof(T), 8 * sizeof(float));
  float* query1 = new float[dim];
  for (size_t i = 0; i < npts; i++) {
    reader.read((char*) query1 , dim * sizeof(float));
    PRINTVEC2(query1 ,dim)
  }
  reader.close();
  EXIT_PRG
  ///TSET
