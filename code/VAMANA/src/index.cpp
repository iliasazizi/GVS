...
#pragma omp parallel for schedule(static, 65536)
   ...
   */
  template<typename T, typename TagT>
  std::pair<unsigned int, unsigned int> Index<T, TagT>::iterate_to_fixed_point(
...
float dist;
    ...
    for (auto it : best_L_nodes) {
      ...
      dist = _distance->compare(query,
                                ...
                                (unsigned) _aligned_dim);
      printf(" K N°%lu  => Distance : %f | Node ID : %u | Time  : %f | TOTAL DC : %lu | Total hops : %lu  \n",
             pos+1,std::sqrt(dist),it.id,t,_NDC,_NHOPS);

//      cout << " K "<<pos+1<<" Distance "<<dist << " id : "<<  it.id << " time : "<< t<<std::endl;
      t = 0;
      _NDC = 0;
      _NHOPS = 0;
      pos++;
