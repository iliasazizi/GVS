...
//used to get neighbors for each node during building
void IndexNSG::get_neighbors(const float *query, const Parameters &parameter,
...
//applied for every node after rng loop
...
#pragma omp for schedule(dynamic, 100)
    for (unsigned n = 0; n < nd_; ++n) {
      ...
      // add neighbors of n to pool
      // sort pool
      // and we do rng on pool, with maximum check to C.
  ...
  while (k < (int)L) {
    ...
    if (retset[k].flag) {
      ...
      for (unsigned m = 0; m < final_graph_[n].size(); ++m) {
        ...
        flags[id] = 1;
        float dist =
            distance_->compare(query, data_ + dimension_ * id, (unsigned)dimension_);_NDC++;
        if (dist >= retset[L - 1].distance) continue;
        Neighbor nn(id, dist, true);
        int r = InsertIntoPool(retset.data(), L, nn);
