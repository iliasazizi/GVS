  ...
  while (k < (int)L) {
    ...
    for(size_t i=0; i < K; i++){
        printf(" K N°%lu  => Distance : %f | Node ID : %u | Time  : %f | TOTAL DC : %i | Total hops : %i  \n",
               i+1,std::sqrt(retset[i].distance),retset[i].id,t,_NDC,_NHOPS);
        _NDC = 0 ;
        t = 0;
        _NHOPS = 0;
...
void IndexSSG::SearchWithOptGraph(const float *query, size_t K,
  ...
  for (unsigned i = 0; i < init_ids.size(); i++) {
    ...
    float norm_x = *x;
    x++;
    float dist = dist_fast->compare(x, query, norm_x, (unsigned)dimension_);_NDC++;
    retset[i] = Neighbor(id, dist, true);
    flags[id] = true;
    L++;
  }
  ...
  while (k < (int)L) {
    ...
    auto distfct = new DistanceL2();

    std::cout << "----------"<<K<<"-NN RESULTS----------- "<<std::endl;
    for(size_t i=0; i < K; i++){
        auto distance = distfct->compare(query, data_ + dimension_ * retset[i].id,
                                           (unsigned)dimension_);
        printf(" K N°%lu  => Distance : %f | Node ID : %u | Time  : %f | TOTAL DC : %i | Total hops : %i  \n",
               i+1,sqrt(distance),retset[i].id,t,_NDC,_NHOPS);
        _NDC=0;
        _NHOPS=0;
        t=0;
    }
